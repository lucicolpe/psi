...
deserunt mollit anim id est laborum."""[init:end]
    ...
    def addCategory(self, noCategories):
        ...
        for i in range (0, noCategories):
            c = Category.objects.get_or_create(name=categories[i], tooltip= "")[0]
            c.save()
            lista_categories.append(categories[i])
    ...
    def addWorkflow(self, noWorkflows):
        ...
        # add apropriate code
        # create fake json
        workflows =["workflow 1","workflow 2","workflow 3","workflow 4","workflow 5","workflow 6","workflow 7","workflow 8","workflow 9","workflow 10","workflow 11","workflow 12","workflow 13"]
        json = getJson()

        for i in range (0, noWorkflows):
            r = random.randint(0,4)
            w = Workflow.objects.get_or_create(name = workflows[i], client_ip="0.0.0.0", json = json)[0]
            w.save()
